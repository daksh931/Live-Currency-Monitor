...
class final :
    ...
    def __init__(self):
        ...
        def fetch():
            ...
            obj_get = SQLLayer()
            allproducts = obj_get.get_products()
            i=0
            for pro in allproducts:
                self.tree.insert("",-1, text= pro.S_no, values=(pro.S_no,pro.Currencies_Name , pro.Date ,pro.Price_USD ))
                i+=1
        ...
        bg = tk.PhotoImage(file = "E:\\4_week training project\Python Project\\blank.png")
        ...
        bg2 = tk.PhotoImage(file = "E:\\4_week training project\Python Project\\blank.png")
    ...
    def Save(self):
        ...
        for x in livevalue:
        # TEsting values
            p.Currencies_Name = list(livevalue.keys())[y]
            p.Date = datetime.datetime.now().strftime(("%Y/%m/%d "))
            p.Price_USD = list(livevalue.values())[y]
    ...
    def dele(self):
        response = tk.messagebox.askyesno("SMS","Do you want to delete selected record?")
        if response == True:

            ref = self.tree.focus()
            pid = self.tree.item(ref,"text")

        
            p = SQLLayer()
            if p.delitem(pid) == True:
                self.tree.delete(ref)
